...
namespace filters {
    ...
    template<typename V>
    class Generator<GeneratorType::RandomHuge2, V> {
      ...
      public:
        Generator(size_t n_elements_build, size_t n_elements_lookup, size_t shared_elements_percentage)
            ...
            size_t stored_n_elements_build = 100000000;
            size_t stored_n_elements_lookup = (n_elements_lookup > 1)
                                              ? 100000000
                                              : 0;

            bool isEmpty = dataContainer.createIfNotExists<GeneratorType::RandomHuge2, V>(stored_n_elements_build,
                    stored_n_elements_lookup, 0);
            build_data = dataContainer.buildData<GeneratorType::RandomHuge2, V>(stored_n_elements_build,
                    stored_n_elements_lookup, 0);
            lookup_data = dataContainer.lookupData<GeneratorType::RandomHuge2, V>(stored_n_elements_build,
                    stored_n_elements_lookup, 0);
            ...
            if (isEmpty) {
                ...
                for (size_t j = 0; j < stored_n_elements_build;) {
                ...
                for (size_t j = shared_elements; j < stored_n_elements_lookup;) {
                    T value = distribution(seed);
                    if (not s.contains(value)) {
                        lookup_data[j] = value;
                        j++;
