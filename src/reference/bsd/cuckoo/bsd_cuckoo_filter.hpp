...
#include <reference/bsd/cuckoo/implementation/bsd_cuckoo_implementation.hpp>
...
namespace filters {
    ...
    struct Filter<FilterType::BSDCuckoo, FP, k, OP> {
        ...
        static_assert(OP::hashingMode == parameter::HashingMode::Mul, "only Mul is supported!");
        ...
        using I = bsd::cuckoo::Implementation<k, static_cast<size_t>(FP::associativity), OP::addressingMode, OP::simd,
                FP::batching>;
        ...
        bool construct(uint32_t *values, size_t length) {
            ...
            init(&histogram);
            bool success = impl->insert(values, length);
            return success;
        }
