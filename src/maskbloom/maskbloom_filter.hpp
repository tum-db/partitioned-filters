...
#include "construct/maskbloom_construct.hpp"
#include "container/maskbloom_container.hpp"
#include "count/bloom_count_horizontal.hpp"

namespace filters {

    template <typename FilterParameter, size_t _k, typename OptimizationParameter>
    struct Filter<FilterType::MaskBloom, FilterParameter, _k, OptimizationParameter> {
        ...
        static_assert(static_cast<size_t>(FP::variant) == k);
        ...
        using Container = maskbloom::MaskBloomContainer<OP::registerSize, OP::partitioning, Vector, Addresser>;
        using Hasher = hash::Hasher<OP::hashingMode, Vector, 0>;
        using PartitionSet = partition::PartitionSet<OP::partitioning, Vector>;

        using Constructor = maskbloom::MaskBloomConstructor<FP::variant, OP::registerSize, OP::simd, Container, Hasher>;
        using Counter = maskbloom::MaskBloomCounter<FP::variant, OP::registerSize, OP::simd, Container, Hasher>;
