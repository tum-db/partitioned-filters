...
namespace kudu {
    ...
    static inline __attribute__ ((always_inline)) __attribute__((__target__("avx2"))) __m256i
    ...
    bool BlockBloomFilter::BucketFindAVX2(const uint32_t bucket_idx, const uint32_t hash) const noexcept {
        const __m256i mask = MakeMask(hash);
