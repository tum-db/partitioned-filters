...
namespace libdivide {
    ...
    static inline struct libdivide_u32_t libdivide_u32_cheap_gen(uint32_t d);
    ...
    *r = (un21 * b + un0 - q0 * v) >> s;
    ...
    static uint64_t
    libdivide_128_div_128_to_64(uint64_t u_hi, uint64_t u_lo, uint64_t v_hi, uint64_t v_lo, uint64_t *r_hi,
                                uint64_t *r_lo) {
    ...
    *r_lo = u_q0v.lo;
    ...
    struct libdivide_u32_t libdivide_u32_cheap_gen(uint32_t d) {
        if (d < 2) {
            d = 2;
        }

        while (d <= -1u) {
            struct libdivide_u32_t div = libdivide_internal_u32_gen(d, 0);
            if (!div.magic || div.more & LIBDIVIDE_ADD_MARKER) {
                d++;
            } else {
                return div;
            }
        }
        LIBDIVIDE_ERROR("Failed to find a cheap magic number.");
    }
